...
- [x] **Narrow mutation allowlist (2026-09-16)**: `MONARCH_MCP_ALLOWED_MUTATIONS` (comma-separated tool names) lets specific entries in `MUTATING_TOOLS` register anyway while read-only mode is otherwise on, instead of an all-or-nothing choice between the 30-tool read-only set and the full 28-tool write surface. An unrecognised name, or one in the code-level `PERMANENTLY_EXCLUDED` set (currently just `delete_transaction` — no visible undo, excluded even if a future config lists it by mistake), is dropped with a log warning rather than erroring: the whole point is that a misconfiguration here can only narrow what's exposed, never widen it. See `read_only.py`'s module docstring and `tests/test_read_only.py::TestAllowedMutations`/the allowlist cases in `TestRegistration`. Currently granted (orchestrator only, see its own `HARDENING.md`): `categorize_transaction`, `mark_transaction_reviewed` — chosen deliberately over also granting `bulk_categorize_transactions`, which applies one category across many transaction IDs per call and was judged a wider blast radius than the stated need.
...
To grant a specific write on top of that (see "Narrow mutation allowlist" above), add `MONARCH_MCP_ALLOWED_MUTATIONS` alongside it, e.g. `"MONARCH_MCP_ALLOWED_MUTATIONS": "categorize_transaction,mark_transaction_reviewed"` — never widen the base `MONARCH_MCP_READ_ONLY` grant itself for a single-tool need.
